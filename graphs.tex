...
What we do then is define $\phi$ and $\psi$ by piecing together all of
the approximations of relevant sets.
\begin{align*}
  \psi(F) &= \{(n, m) \mid m \in F(\set{n})^*\} \cup \{0\}\\
  \phi(d) &= X \mapsto
    \bigcup \{\set{m} \mid \exists n \in X^*.\ (n, m) \in d\}
\end{align*}

So $\psi$ creates a table mapping a fragment of input to the
...
each other. Is it the case that $\psi \circ \phi = 1$ for example?

Suppose we have some $X \in \mathcal{P}(\mathbb{N})$, we wish to show
that $\psi(\phi(X)) = X$. If $(n, m) \in \psi(\phi(X))$ then
$m \in (\phi(X))(n)^*$. In order to show that
$\psi(\phi(X)) \supseteq X$ all we need do then is show that
$(n, m) \in X$ implies $m \in (\phi(X))(n)^*$. Suppose we have such an
$(n, m)$. We have immediately run into a problem: $(\phi(X))(n)$ will
not necessarily contain

 then $m = \bigcup_i \set{m_i}$ where for each $m_i$ there's
a $n_i \in n^*$ so that $(m_i, n_i) \in X$.

Now we have a problem, we may know that we have a collection of
approximations of $(n, m)$ in $X$, but we have no way of converting
this information into evidence for $(n, m) \in X$.  It's only true if
we can say something like if $(n_i, m_i) \in X$ and
$\cup n_i \subseteq n$, then $(n, \cup m_i) \in X$. But how could we
know such a thing? We'd need to make some restrictions on how $X$ came
to be.
